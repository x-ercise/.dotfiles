...
function activate(context) {
    ...
    context.subscriptions.push(vscode_1.languages.registerCodeActionsProvider({ language: 'json', scheme: 'file' }, new NpmCodeActionProvider()));
}
...
function runNpmCommandInPackages(command, allowAll = false, alwaysRunInputWindow = false, dirs) {
    ...
    if (descriptions.length === 0) {
        vscode_1.window.showErrorMessage("No scripts found.", { modal: true });
        return;
    }
